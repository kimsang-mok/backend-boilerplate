...
export default abstract class Controller {
  ...
  async index(req: Request): Promise<ResponseObject> {
    ...
    const perpage: number = Number(req.query.perpage || 20);
    const relations: string = String(req.query.relations || "");
    const where: string = String(req.query.where || "");
    const whereValue: string = String(req.query.where_value || "");
    const offset: number = (page - 1) * perpage;
    const orderBy: string = String(req.query.order_by || "");
    const isDesc: boolean = req.query.desc == "true";
    const groupBy: string = String(req.query.group_by || "");
  ...
  async find(id: number | string, req: Request): Promise<ResponseObject> {
    const relations: string = String(req.query.relations || "");
    const response: ResponseObject = {
      status_code: 200,
      message: locale.__("Success"),
  ...
  async destroy(id: number | string): Promise<ResponseObject> {
    const response: ResponseObject = {
      status_code: 200,
  ...
  async update(id: number | string, req: Request): Promise<ResponseObject> {
    const response: ResponseObject = {
      status_code: 200,
